import type { Station, Train, RouteStation, CoachComposition, Seat, SeatLayout, LiveTrainStatus, Passenger, Booking, User, SavedPassenger, SavedPaymentMethod, TrainStatus, SeatType, BookingStatus } from '@/types';
...
function generateSeatLayout(totalSeats: number, classCode: string): SeatLayout {
  const seats: Seat[] = [];
  let seatNumber = 1;
  ...
  if (classCode === '1A') {
    ...
    for (let row = 0; row < rows; row++) {
      const types: SeatType[] = ['LB', 'UB', 'LB', 'UB'];
  ...
  if (classCode === '2A') {
    ...
    for (let row = 0; row < rows; row++) {
      const types: SeatType[] = ['LB', 'MB', 'UB', 'LB', 'MB', 'UB'];
  ...
  if (classCode === '3A' || classCode === 'SL') {
    ...
    for (let row = 0; row < rows; row++) {
      const types: SeatType[] = ['LB', 'MB', 'UB', 'SL', 'SU', 'LB', 'MB', 'UB'];
